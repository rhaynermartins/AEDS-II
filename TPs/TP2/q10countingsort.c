...
// encontra a maior capacidade do vetor
int maior_capacidade(Restaurante** array, int n) {
    int maior = array[0]->capacidade; // assume a primeira capacidade como maior
    int i; // indice de repeticao

    for (i = 1; i < n; i++) { // percorre o resto do vetor
        if (array[i]->capacidade > maior) { // se encontrar capacidade maior
            maior = array[i]->capacidade; // atualiza o maior valor
        }
    }

    return maior; // retorna a maior capacidade
}

// counting sort por capacidade
void counting_sort(Restaurante** array, int n, long long* comparacoes, long long* movimentacoes) {
    int i; // indice de repeticao
    int max = maior_capacidade(array, n); // descobre a maior capacidade do vetor

    int* count = (int*) malloc((max + 1) * sizeof(int)); // cria o vetor de contagem
    Restaurante** ordenado = (Restaurante**) malloc(n * sizeof(Restaurante*)); // cria o vetor ordenado

    for (i = 0; i <= max; i++) { // inicializa o vetor de contagem com zero
        count[i] = 0; // zera a posicao atual
    }

    for (i = 0; i < n; i++) { // conta quantas vezes cada capacidade aparece
        count[array[i]->capacidade]++; // soma 1 na capacidade correspondente
        (*movimentacoes)++; // conta a leitura/uso do elemento
    }

    for (i = 1; i <= max; i++) { // transforma o vetor de contagem em acumulado
        count[i] += count[i - 1]; // soma com a posicao anterior
    }

    for (i = n - 1; i >= 0; i--) { // percorre de tras para frente para manter estabilidade
        ordenado[count[array[i]->capacidade] - 1] = array[i]; // coloca o elemento na posicao correta
        count[array[i]->capacidade]--; // diminui a contagem da capacidade usada
        (*movimentacoes)++; // conta a movimentacao para o vetor ordenado
    }

    for (i = 0; i < n; i++) { // copia o vetor ordenado de volta para o vetor original
        array[i] = ordenado[i]; // copia o elemento
        (*movimentacoes)++; // conta a movimentacao
    }

    free(count); // libera o vetor de contagem
    free(ordenado); // libera o vetor auxiliar
}

// grava arquivo de log
void criar_log(long long comparacoes, long long movimentacoes) { 
    FILE* log = fopen(MATRICULA "_countingsort.txt", "w"); // abre o arquivo no modo escrita
...
int main() { 
    ...
    if (quantidade > 0) { // se existir pelo menos um restaurante
        counting_sort(selecionados, quantidade, &comparacoes, &movimentacoes); // ordena com counting sort
    }
