...
// classe principal
public class q9heapsort {
    public static final String MATRICULA = "898128"; // guarda a matricula usada no log

    // compara duas datas e em caso de empate compara o nome
    public static int compararRestaurantes(Restaurante a, Restaurante b, long[] comparacoes) {
        comparacoes[0]++; // conta uma comparacao pelo ano

        if (a.getDataAbertura().getAno() != b.getDataAbertura().getAno()) { // se os anos forem diferentes
            return a.getDataAbertura().getAno() - b.getDataAbertura().getAno(); // compara pelo ano
        }

        comparacoes[0]++; // conta uma comparacao pelo mes

        if (a.getDataAbertura().getMes() != b.getDataAbertura().getMes()) { // se os meses forem diferentes
            return a.getDataAbertura().getMes() - b.getDataAbertura().getMes(); // compara pelo mes
        }

        comparacoes[0]++; // conta uma comparacao pelo dia

        if (a.getDataAbertura().getDia() != b.getDataAbertura().getDia()) { // se os dias forem diferentes
            return a.getDataAbertura().getDia() - b.getDataAbertura().getDia(); // compara pelo dia
        }

        comparacoes[0]++; // conta uma comparacao pelo nome
    ...
    // refaz a estrutura de heap
    public static void reconstruir(Restaurante[] array, int tamHeap, int i, long[] comparacoes, long[] movimentacoes) {
        int maior = i; // assume que a raiz atual é o maior
        int esq = 2 * i + 1; // calcula o indice do filho esquerdo
        int dir = 2 * i + 2; // calcula o indice do filho direito

        if (esq < tamHeap && compararRestaurantes(array[esq], array[maior], comparacoes) > 0) { // se o filho esquerdo existir e for maior
            maior = esq; // atualiza o indice do maior
        }

        if (dir < tamHeap && compararRestaurantes(array[dir], array[maior], comparacoes) > 0) { // se o filho direito existir e for maior
            maior = dir; // atualiza o indice do maior
        }

        if (maior != i) { // se a raiz nao for o maior elemento
            swap(array, i, maior, movimentacoes); // troca a raiz com o maior filho
            reconstruir(array, tamHeap, maior, comparacoes, movimentacoes); // continua reconstruindo o heap
        }
    }
    ...
    public static void heapsort(Restaurante[] array, int n, long[] comparacoes, long[] movimentacoes) {
        ...
        for (i = n / 2 - 1; i >= 0; i--) { // constroi o heap inicial
            reconstruir(array, n, i, comparacoes, movimentacoes); // organiza a subarvore de indice i
        }

        for (i = n - 1; i > 0; i--) { // vai colocando o maior elemento no final
            swap(array, 0, i, movimentacoes); // troca a raiz com o ultimo elemento do heap
            reconstruir(array, i, 0, comparacoes, movimentacoes); // refaz o heap no trecho restante
        }
    }
    ...
    public static void criarLog(long comparacoes, long movimentacoes, double tempo) {
        try {
            PrintWriter pw = new PrintWriter(MATRICULA + "_heapsort.txt"); // cria o arquivo de log
    ...
    public static void main(String[] args) {
        ...
        Restaurante[] selecionados = new Restaurante[1000]; // vetor para guardar os restaurantes escolhidos
        ...
        if (quantidade > 0) { // se existir pelo menos um restaurante
            heapsort(selecionados, quantidade, comparacoes, movimentacoes); // ordena por data de abertura e desempata por nome
        }
