...
// classe FilaCircular
class FilaCircular {
    private Restaurante[] array; // vetor da fila
    private int primeiro; // aponta para o primeiro elemento
    private int ultimo; // aponta para a proxima posicao livre

    // construtor da fila circular
    public FilaCircular(int tamanho) {
        this.array = new Restaurante[tamanho + 1]; // cria vetor com uma posicao extra para controle circular
        this.primeiro = 0; // começa no indice 0
        this.ultimo = 0; // começa no indice 0
    }

    // verifica se a fila esta vazia
    public boolean vazia() {
        return this.primeiro == this.ultimo; // retorna true se primeiro e ultimo forem iguais
    }

    // verifica se a fila esta cheia
    public boolean cheia() {
        return ((this.ultimo + 1) % this.array.length) == this.primeiro; // retorna true se a proxima posicao de ultimo for primeiro
    }
    ...
    public Restaurante remover() throws Exception {
        ...
        Restaurante resp = this.array[this.primeiro]; // guarda o elemento que sera removido
        this.primeiro = (this.primeiro + 1) % this.array.length; // avanca circularmente o primeiro
        return resp; // retorna o restaurante removido
    }

    // insere um restaurante no fim da fila
    public void inserir(Restaurante restaurante) throws Exception {
        if (cheia()) { // se a fila estiver cheia
            throw new Exception("Erro ao inserir"); // nao remove aqui
        }

        this.array[this.ultimo] = restaurante; // coloca o restaurante na posicao de ultimo
        this.ultimo = (this.ultimo + 1) % this.array.length; // avanca circularmente o ultimo
    }

    // calcula a media arredondada do ano de abertura
    public int mediaAnoAbertura() {
        int soma = 0; // soma dos anos
        int count = 0; // quantidade de elementos
        int i = this.primeiro; // começa do primeiro

        while (i != this.ultimo) { // percorre a fila ate o ultimo
            soma += this.array[i].getDataAbertura().getAno(); // soma o ano atual
            count++; // aumenta a quantidade
            i = (i + 1) % this.array.length; // avanca circularmente
        }

        return (int) Math.round((double) soma / count); // calcula a media arredondada
    }

    // mostra a fila do primeiro ao ultimo
    public void mostrar() {
        int i = this.primeiro; // começa do primeiro
        int pos = 0; // posicao logica para imprimir

        while (i != this.ultimo) { // percorre a fila ate o ultimo
            System.out.println("[" + pos + "] " + this.array[i].formatar()); // imprime a posicao e o restaurante
            pos++; // aumenta a posicao logica
            i = (i + 1) % this.array.length; // avanca circularmente
        }
    }
}
...
public class q13filacircular {
    public static void main(String[] args) throws Exception {
        ...
        FilaCircular fila = new FilaCircular(5); // cria a fila circular de tamanho 5
        ...
        while (id != -1) { // enquanto nao encontrar -1
            ...
            if (restaurante != null) { // se encontrou
                if (fila.cheia()) { // se a fila estiver cheia
                    Restaurante removido = fila.remover(); // remove antes de inserir
                    System.out.println("(R)" + removido.getNome()); // imprime o removido
                }

                fila.inserir(restaurante); // insere na fila
                System.out.println("(I)" + fila.mediaAnoAbertura()); // imprime a media apos a insercao
            }
        ...
        for (int i = 0; i < n; i++) { // percorre os n comandos
            ...
            if (comando.compareTo("I") == 0) { // se o comando for inserir
            ...
            } else if (comando.compareTo("R") == 0) { // se o comando for remover
                Restaurante removido = fila.remover(); // remove da fila
                System.out.println("(R)" + removido.getNome()); // imprime o nome removido sem espaco
            }

            scLinha.close(); // fecha o Scanner da linha
        }

        fila.mostrar(); // mostra a fila final
