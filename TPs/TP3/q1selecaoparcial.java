...
class Restaurante {
    ...
    public static Restaurante parseRestaurante(String s) {
        ...
        boolean aberto = sc.nextBoolean(); // le se esta aberto

        sc.close(); // fecha o Scanner da linha

        String[] tiposCozinha = parseTiposCozinha(tipos); // transforma os tipos em vetor
        int faixaPreco = parseFaixaPreco(faixa); // transforma a faixa em numero

        Scanner scHorario = new Scanner(horario); // cria Scanner para separar o horario
        scHorario.useDelimiter("-"); // define "-" como separador

        String abertura = scHorario.next(); // le a abertura
        String fechamento = scHorario.next(); // le o fechamento

        scHorario.close(); // fecha o Scanner do horario

        Hora horarioAbertura = Hora.parseHora(abertura); // cria a hora de abertura
        Hora horarioFechamento = Hora.parseHora(fechamento); // cria a hora de fechamento
        Data dataAbertura = Data.parseData(data); // cria a data de abertura
    ...
    public String formatarTiposCozinha() {
        ...
        return resp; // retorna os tipos formatados
    }
...
// classe que guarda todos os restaurantes do csv
class ColecaoRestaurantes {
    private int tamanho; // guarda a quantidade de restaurantes
    private Restaurante[] restaurantes; // vetor de restaurantes

    // construtor padrao
    public ColecaoRestaurantes() {
        this.tamanho = 0; // começa com tamanho zero
    ...
    public void lerCsv(String path) {
        try {
            File arquivo = new File(path); // cria o objeto do arquivo
            ...
            for (int i = 0; i < this.tamanho; i++) { // percorre as linhas
                String linha = sc.nextLine(); // le uma linha
    ...
    // le o csv padrao em /tmp/restaurantes.csv
    public static ColecaoRestaurantes lerCsv() {
        ColecaoRestaurantes colecao = new ColecaoRestaurantes(); // cria a colecao
        colecao.lerCsv("/tmp/restaurantes.csv"); // le o arquivo padrao
        return colecao; // retorna a colecao
    }
