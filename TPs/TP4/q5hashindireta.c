...
#define TAM_TABELA 31 // tamanho da tabela hash
...
// celula da lista simples
typedef struct Celula {
    Restaurante* restaurante; // guarda o restaurante
    struct Celula* prox; // aponta para a proxima celula
} Celula;

// tabela hash indireta
typedef struct TabelaHash {
    Celula* tabela[TAM_TABELA]; // vetor de listas
...
void parse_tipos_cozinha(char* s, Restaurante* restaurante) {
    ...
    int indice_tipo = 0; // guarda indice do tipo

    restaurante->quantidade_tipos = contar_tipos_cozinha(s); // salva quantidade

    while (s[i] != '\0') { // percorre texto
        if (s[i] == ';') { // se achar separador
            restaurante->tipos_cozinha[indice_tipo][j] = '\0'; // fecha tipo atual
...
Celula* nova_celula(Restaurante* restaurante) {
    Celula* nova = (Celula*) malloc(sizeof(Celula)); // aloca celula
...
void iniciar_hash(TabelaHash* hash) {
    ...
    for (i = 0; i < TAM_TABELA; i++) { // percorre tabela
        hash->tabela[i] = NULL; // inicia lista vazia
    }

    hash->comparacoes = 0; // zera comparacoes
}
...
// funcao hash
int h(char* nome) {
    return soma_ascii(nome) % TAM_TABELA; // retorna posicao
}

// insere no fim da lista
void inserir_fim_lista(Celula** lista, Restaurante* restaurante) {
    Celula* nova = nova_celula(restaurante); // cria nova celula

    if (*lista == NULL) { // se lista vazia
        *lista = nova; // nova vira primeira
    } else {
        Celula* atual = *lista; // comeca no inicio

        while (atual->prox != NULL) { // vai ate o fim
            atual = atual->prox; // avanca
        }

        atual->prox = nova; // liga nova no final
    }
}

// insere restaurante na hash
void inserir_hash(TabelaHash* hash, Restaurante* restaurante) {
    int pos = h(restaurante->nome); // calcula posicao

    inserir_fim_lista(&hash->tabela[pos], restaurante); // insere na lista da posicao
}

// pesquisa restaurante na hash
int pesquisar_hash(TabelaHash* hash, char* nome, Restaurante** encontrado) {
    int pos = h(nome); // calcula posicao
    Celula* atual = hash->tabela[pos]; // pega lista da posicao

    while (atual != NULL) { // percorre lista
        hash->comparacoes++; // conta comparacao

        if (comparar_texto(atual->restaurante->nome, nome) == 0) { // se encontrou
            *encontrado = atual->restaurante; // guarda restaurante encontrado
            return pos; // retorna posicao da tabela
        }

        atual = atual->prox; // avanca na lista
    }

    *encontrado = NULL; // nao encontrou
    return -1; // retorna -1
}

// imprime resultado da pesquisa
void imprimir_pesquisa(TabelaHash* hash, char* nome) {
    Restaurante* encontrado; // guarda restaurante encontrado
    int pos = pesquisar_hash(hash, nome, &encontrado); // pesquisa
    char buffer[1000]; // buffer de formatacao

    if (pos == -1) { // se nao encontrou
        printf("-1\n"); // imprime -1
    } else {
        formatar_restaurante(encontrado, buffer); // formata restaurante
...
void criar_log(long long comparacoes) {
    FILE* log = fopen(MATRICULA "_hash_indireta.txt", "w"); // abre log
...
int main() {
    ...
    while (id != -1) { // enquanto nao for -1
        ...
        if (restaurante != NULL) { // se encontrou
            inserir_hash(&hash, restaurante); // insere na hash
        }
    ...
    while (!is_fim(nome)) { // enquanto nao for FIM
        imprimir_pesquisa(&hash, nome); // pesquisa e imprime
